...
datum/reagent/salglu_solution
	...
	id = "salglu_solution"
	description = "A medical solution that slowly restores vital powers and blood."
	reagent_state = LIQUID
...
datum/reagent/salglu_solution/on_mob_life(var/mob/living/M as mob)
	...
	if(prob(33))
		...
		M.adjustFireLoss(-2*REM)
	if(prob(20))
		M.reagents.add_reagent("blood",pick(1,3), list("donor"=M,"viruses"=null,"blood_DNA"=M.dna.unique_enzymes,"blood_type"=M.dna.b_type,"resistances"=null,"trace_chem"=null))
		M.reagents.remove_reagent("salglu_solution",3)
	..()
	return
...
/datum/chemical_reaction/salglu_solution
	...
	required_reagents = list("sodiumchloride" = 1, "water" = 1, "sugar" = 1) // IRL that propotions will kill you, but this is game.
	result_amount = 3
...
/datum/chemical_reaction/synthflesh_structurization
	name = "Synthflesh Structurization"
	id =  "synststruct"
	result = null
	required_reagents = list("synthflesh" = 2, "clonexadone" = 1)
	result_amount = 1

/datum/chemical_reaction/synthflesh_structurization/on_reaction(var/datum/reagents/holder, var/created_volume)
...
See code\modules\reagents\reagents\vg.dm 6
...
datum/reagent/sal_acid/on_mob_life(var/mob/living/M as mob)
	...
	if(M.bodytemperature < 170)
		...
		if(M.getBruteLoss()) M.adjustBruteLoss(-1*REM)
...
/datum/reagent/mutadone/on_mob_life(var/mob/living/carbon/human/M as mob) //RYETALYN AS FUCK
	if(!holder) return
	if(!M) M = holder.my_atom
