...
/datum/chemical_reaction/sonic_powder_deafen/on_reaction(var/datum/reagents/holder, var/created_volume)
	...
	for(var/mob/living/M in hearers(5, location))
		var/ear_safety = 0
		var/distance = max(1,get_dist(src,location))
		if(iscarbon(M))
			var/mob/living/carbon/C = M
			if(ishuman(C))
				var/mob/living/carbon/human/H = C
				if (H.is_on_ears(/obj/item/clothing/ears/earmuffs) || istype(H.head, /obj/item/clothing/head/helmet) || M_HULK in H.mutations)
					ear_safety++
