...
/obj/item/weapon/reagent_containers/pill/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(M == user) //Someone swallowing a pill
		user.visible_message("<span class='notice'>[user] [apply_method] \the [src].</span>", \
		...
		user.visible_message("<span class='warning'>[user] forces [M] to [apply_method] \the [src].</span>", \
		"<span class='notice'>You force [M] to [apply_method] \the [src].</span>")
