...
datum/reagent/vomit/reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)
	if(method==INGEST)
...
datum/reagent/egg/on_mob_life(var/mob/M)
	if(!M) M = holder.my_atom

	..()
	return

datum/reagent/egg/reaction_turf(var/turf/T, var/volume)
	if(!istype(T, /turf/space))
...
datum/reagent/slinetoxin/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/aslimetoxin/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/srejuvenate/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/inaprovaline/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/space_drugs/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/serotrotium/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
/*
...
datum/reagent/mercury/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/chlorine/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/fluorine/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/lithium/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/ryetalyn/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/tricordrazine/on_mob_life(var/mob/living/M as mob)
	...
	if(M.getBruteLoss()) M.heal_organ_damage(1*REM,0)
	if(M.getFireLoss()) M.heal_organ_damage(0,1*REM)
...
datum/reagent/synaptizine/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/impedrezene/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/hyronalin/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/arithrazine/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/imidazoline/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/inacusiate/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/peridaxon/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/cloneoxadone/on_mob_life(var/mob/living/M as mob)
	if(!holder) return
	if(!M) M = holder.my_atom
	if(M.bodytemperature < 170)
		M.adjustCloneLoss(-3)
...
datum/reagent/rezadone/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/spaceacillin/overdose_process(var/mob/living/M as mob)
	M.adjustToxLoss(1*REM)
	..()
	return
...
datum/reagent/spysauce/on_mob_life(var/mob/living/M as mob)
	if(!M) M = holder.my_atom
...
datum/reagent/caspaicin/on_mob_life(var/mob/living/M as mob)
	if(!holder) return
	if(!M) M = holder.my_atom
	if(!data) data = 1
	switch(data)
...
datum/reagent/sodiumchloride/on_mob_life(var/mob/living/M as mob)
	if(!M) M = holder.my_atom
	M.hydration -= hydration_factor
...
datum/reagent/drink/watermelonjuice/on_mob_life(var/mob/living/M as mob)
	if(!holder) return
	..()
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		if(H.s_tone >= 100) //Nigger
			if(prob(20))
...
datum/reagent/drink/hot_coco/on_mob_life(var/mob/living/M as mob)
	if(!holder) return
	if (M.bodytemperature < 310)//310 is the normal bodytemp. 310.055
...
datum/reagent/drink/cofee/on_mob_life(var/mob/living/M as mob)
	if(!holder) return
	..()
	if(!holder)
		holder = M.reagents
...
datum/reagent/drink/cofee/icecoffee
	name = "Iced Coffee"
	id = "icecoffee"
...
datum/reagent/drink/cofee/soy_latte
	name = "Soy Latte"
	id = "soy_latte"
...
datum/reagent/drink/cofee/soy_latte/on_mob_life(var/mob/living/M as mob)
	..()
	M.sleeping = 0
	if(M.getBruteLoss() && prob(20)) M.heal_organ_damage(1,0)
	return

datum/reagent/drink/cofee/cafe_latte
	name = "Cafe Latte"
	id = "cafe_latte"
...
datum/reagent/drink/cofee/cafe_latte/on_mob_life(var/mob/living/M as mob)
		if(!holder) return
		..()
		M.sleeping = 0
		if(M.getBruteLoss() && prob(20)) M.heal_organ_damage(1,0)
