...
/obj/machinery/computer/library/checkout/Topic(href, href_list)
	...
	if(href_list["delbyckey"])
		...
		if(ans=="Yes")
			var/DBQuery/query = dbcon.NewQuery("DELETE FROM library WHERE ckey='[sanitizeSQL(tckey)]'")
			var/response = query.Execute()
			if(!response)
				usr << query.ErrorMsg()
				return
			...
			if(scanner.cache)
				...
				if(choice == "Confirm")
					...
					if(!dbcon.IsConnected())
						...
						var/DBQuery/query = dbcon.NewQuery("INSERT INTO library (author, title, content, category, ckey) VALUES ('[sqlauthor]', '[sqltitle]', '[sqlcontent]', '[sqlcategory]', '[ckey(usr.key)]')")
						var/response = query.Execute()
						if(!response)
							usr << query.ErrorMsg()
						else
