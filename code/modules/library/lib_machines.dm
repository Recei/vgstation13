...
/datum/library_catalog/proc/rmBookByID(var/mob/user, var/id as text)
	...
	if(!sqlid)
		return
	var/DBQuery/query = dbcon.NewQuery("DELETE FROM library WHERE id=[sqlid]")
	query.Execute()
...
/datum/library_catalog/proc/getBookByID(var/id as text)
	...
	if(!sqlid)
		return
	var/DBQuery/query = dbcon.NewQuery("SELECT  id, author, title, category, ckey  FROM library WHERE id=[sqlid]")
	query.Execute()
