...
/datum/surgery_step/cavity/make_space/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	user.visible_message("<span class = 'blue'> [user] makes some space inside [target]'s [get_cavity(affected)] cavity with \the [tool].</span>",
	"<span class = 'blue'> You make some space inside [target]'s [get_cavity(affected)] cavity with \the [tool].</span>" )
...
/datum/surgery_step/cavity/close_space/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/datum/organ/external/chest/affected = target.get_organ(target_zone)
	user.visible_message("<span class = 'blue'> [user] mends [target]'s [get_cavity(affected)] cavity walls with \the [tool].</span>",
	"<span class = 'blue'> You mend [target]'s [get_cavity(affected)] cavity walls with \the [tool].</span>" )
...
/datum/surgery_step/cavity/place_item/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	user.visible_message("<span class = 'blue'> [user] puts \the [tool] inside [target]'s [get_cavity(affected)] cavity.</span>",
	"<span class = 'blue'> You put \the [tool] inside [target]'s [get_cavity(affected)] cavity." )
	if (tool.w_class > get_max_wclass(affected)/2 && prob(50))
		user << "<span class = 'warning'> You tear some vessels trying to fit such big object in this cavity.</span>"
...
/datum/surgery_step/cavity/implant_removal/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (affected.implants.len)
		...
		if (prob(find_prob))
			user.visible_message("<span class = 'blue'> [user] takes something out of incision on [target]'s [affected.display_name] with \the [tool].</span>",
			"<span class = 'blue'> You take [obj] out of incision on [target]'s [affected.display_name]s with \the [tool].</span>" )
			...
			user.visible_message("<span class = 'blue'> [user] removes \the [tool] from [target]'s [affected.display_name].</span>", \
			"<span class = 'blue'> There's something inside [target]'s [affected.display_name], but you just missed it this time.</span>" )
	else if (affected.hidden)
		user.visible_message("<span class = 'blue'> [user] takes something out of incision on [target]'s [affected.display_name] with \the [tool].</span>", \
		"<span class = 'blue'> You take something out of incision on [target]'s [affected.display_name]s with \the [tool].</span>" )
		...
		user.visible_message("<span class = 'blue'> [user] could not find anything inside [target]'s [affected.display_name], and pulls \the [tool] out.</span>",
		"<span class = 'blue'> You could not find anything inside [target]'s [affected.display_name].</span>" )
