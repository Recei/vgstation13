...
/obj/machinery/door/poddoor/shutters/floor
	name = "Floor mounted shutters"
	density = 0
	opacity = 0
...
/obj/machinery/door/poddoor/shutters/floor/open()
	if(operating == 1) //doors can still open when emag-disabled
		return
	if(!ticker)
		return 0
...
/obj/machinery/door/poddoor/shutters/floor/close()
	if(operating)
		return
	operating = 1
	flick("shutterc1", src)
