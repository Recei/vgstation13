...
/datum/intercept_text/proc/build_gang(datum/mind/correct_person)
	src.text += {"<BR><BR>Syndicate forces may be supplying opposing criminal organizations within your sector in an attempt at destabilizing our operations."
		Ensure law and order is maintained on the station and be on the lookout for violent confrontations between station factions.
		Forced labor on the mining asteroid is recommended for anyone convicted of heading a criminal organization."}
