...
/datum/game_mode/gang/pre_setup()
	...
	var/list/datum/mind/possible_gangs = get_players_for_role(ROLE_GANG)

	for(var/datum/mind/player in possible_gangs)
		for(var/job in restricted_jobs)//Removing heads and such from the list
			if(player.assigned_role == job)
				possible_gangs -= player

	if(possible_gangs.len >= 2)
		assign_bosses(possible_gangs)
		if(possible_gangs.len > 20)
			assign_bosses(possible_gangs)
...
/datum/game_mode/gang/proc/assign_bosses(var/list/possible_gangs)
	var/datum/mind/boss = pick(possible_gangs)
	A_bosses += boss
	possible_gangs -= boss
	boss.special_role = "[gang_name("A")] Gang (A) Boss"
	log_game("[boss.key] has been selected as a boss for the [gang_name("A")] Gang (A)")

	boss = pick(possible_gangs)
	B_bosses += boss
	possible_gangs -= boss
...
/datum/game_mode/proc/add_gangster(datum/mind/gangster_mind, var/gang, var/check = 1)
	if((gangster_mind in A_bosses) || (gangster_mind in A_gangsters) || (gangster_mind in B_bosses) || (gangster_mind in B_gangsters))
		return -1 //You can't convert your enemy

	var/mob/living/carbon/human/H = gangster_mind.current

	if(jobban_isbanned(H, "gangster"))
		return -2 //Or jobbanned person
	...
	for(var/obj/item/weapon/implant/loyalty/L in H) // check loyalty implant in the contents
		if(L.imp_in == H) // a check if it's actually implanted
			return -3 //Or implanted NT member
	...
	log_admin("\[[time_stamp()]\] <font color='red'>Has been converted to the [gang=="A" ? "[gang_name("A")] Gang (A)" : "[gang_name("B")] Gang (B)"]!</font>")
...
/datum/game_mode/proc/remove_gangster(datum/mind/gangster_mind, var/beingborged, var/silent)
	...
	if(silent < 2)
		log_admin("\[[time_stamp()]\] <font color='red'>Has reformed and defected from the [gang=="A" ? "[gang_name("A")] Gang (A)" : "[gang_name("B")] Gang (B)"]!</font>")
